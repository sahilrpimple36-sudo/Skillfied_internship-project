"""
╔══════════════════════════════════════════════════════════════╗
║        NetGuard — ARP Spoof Tester (Windows Only)           ║
║   Run as Administrator!  python arp_spoof_tester.py          ║
╚══════════════════════════════════════════════════════════════╝

HOW IT WORKS:
  1. Scans your network to find real devices
  2. Learns their real IP → MAC mappings
  3. Sends spoofed ARP packets pretending to be another device
  4. NetGuard detects the MAC change and fires an alert in the GUI

REQUIREMENT: Run this script as Administrator
"""
...
conf.verb = 0   # silence all scapy output
...
def get_windows_interfaces():
    """Return list of (raw_name, ip) for all interfaces that have a real IP."""
    results = []
...
def get_subnet(ip):
    """Turn 192.168.1.55 into 192.168.1.0/24"""
    parts = ip.split(".")
    return f"{parts[0]}.{parts[1]}.{parts[2]}.0/24"


def scan_network(iface, subnet):
    """ARP-scan the subnet and return list of {ip, mac} dicts."""
...
def send_arp_reply(iface, src_ip, src_mac, dst_ip="255.255.255.255"):
    """Send a single ARP reply packet."""
...
def pick_target(devices, my_ip):
    """Let user pick which discovered device to spoof."""
...
def mode_quick_test(iface):
    """
    Quick test using completely made-up IPs and MACs.
    No real network traffic is disturbed.
    Perfect for testing NetGuard without touching real devices.
    """
    FAKE_IP       = "10.99.88.77"          # made-up IP (not real on your LAN)
    REAL_MAC      = "11:22:33:44:55:66"    # "original" MAC we'll teach NetGuard
    ATTACKER_MAC  = "aa:bb:cc:dd:ee:ff"    # different MAC sent after (triggers alert)
...
def mode_scan_and_spoof(iface, my_ip):
    """
    Scan the real network, pick a device, and spoof it.
    NetGuard will detect the MAC change on the Windows GUI.
    """
    ...
    # Get our own MAC to use as the "attacker" MAC
    try:
...
def mode_continuous_spoof(iface, my_ip):
    """
    Continuously send spoofed ARPs every 2 seconds.
    Keeps triggering alerts — good for sustained testing.
...
def mode_custom(iface):
    """
    Manually enter any IP and MACs you want.
    Full control for advanced testing.
    """
