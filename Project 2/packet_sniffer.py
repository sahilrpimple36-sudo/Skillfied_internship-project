...
sniffer_thread = None
MAX_DISPLAY = 5000
CAPTURE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'captures', 'session.pcap')
...
def _process_packet(pkt):
    ...
    try:
        ...
        entry = {'time': datetime.now().strftime('%H:%M:%S'), 'src': src, 'dst': dst, 'protocol': proto, 'size': size, 'info': info}
...
def start_sniffing(interface, filter_str=''):
    ...
    def _run():
        ...
        try:
            sniff(iface=interface, prn=_process_packet, store=False, stop_filter=lambda x: not sniffer_running, filter=filter_str if filter_str else None)
        except Exception:
            pass
        finally:
            try:
