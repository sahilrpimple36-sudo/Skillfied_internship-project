...
# ── Human-readable interface name mapping ────────────────────────────────────
def _friendly_name(raw_name, ip):
    """
    Convert a raw OS interface name into a readable label.
    Examples:
      eth0 / Ethernet0  → Wired Network (Ethernet)
      wlan0 / Wi-Fi     → Wi-Fi / Wireless
      lo / loopback     → Loopback (testing only)
...
def get_interfaces():
    """Return a list of interfaces with human-readable display names."""
    try:
        ...
        for iface in get_if_list():
            try:
                ...
                ip = "N/A"
            interfaces.append({
                "name":    iface,           # raw name sent to the backend
                "ip":      ip,
                "label":   _friendly_name(iface, ip)   # human-readable label
            })
        return interfaces
...
def get_protocol_name(packet):
    try:
        ...
        if packet.haslayer(TCP):
            ...
            if 443 in (sport, dport): return "HTTPS"
            if 22  in (sport, dport): return "SSH"
            if 21  in (sport, dport): return "FTP"
